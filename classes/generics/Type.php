<?
require_once(dirname(__FILE__).'/IGeneric.php');
...
class Type implements IEquatable {
	...
	private static function generateType(array &$stringArray, &$pos) {
		...
		if (self::isType($name) == false) throw new InvalidArgumentException("The type is invalid1!");
		...
		if (class_exists($name)) {
			...
			if ($class->implementsInterface('IGeneric')) {
				...
				if ($genericTypesCount != 0) throw new Exception("The type $name is not generic!");
			}
		} else {
			if ($genericTypesCount != 0) throw new Exception("The type $name is not generic!");
		}
	...
	private static function isType($type) {
		if (is_string($type)) {
			if (array_key_exists($type, self::$functions)) return true;
			if (class_exists($type, true)) return true;
			if (interface_exists($type, true)) return true;
		}
		return false;
	}
	...
	public function IsInterface() {
		if ($this->reflection == null) return false;
	...
	public function IsSubclassOf(Type $type) {
		...
		if ($type->IsGeneric()) return false;
		if ($this->reflection->isSubclassOf($type->reflection) == false) return false;
		return true;
	}
	...
	public function IsItemFromType($item) {
		...
		if ($this->IsPrimitive()) {
			if ($type == Type::NUMBER) {
				if (is_string($item) == true) return false;
			}
			$func = self::$functions[$type];
			return $func($item);
		}
		$itemType = Type::TypeOf($item);
		if ($this->Equals($itemType)) return true;
		if ($itemType->IsSubclassOf($this)) return true;
		return false;
	}
	...
	public function Equals($obj) {
		...
		if ($obj->GetGenericTypesCount() != $genericsCount) return false;
		if ($genericsCount > 0) {
