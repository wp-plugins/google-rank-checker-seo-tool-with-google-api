<?
require_once(OPT7_GRC_PLUGINPATH. '/classes/google/Api/Ads/AdWords/Lib/AdWordsUser.php');
require_once(OPT7_GRC_PLUGINPATH. '/classes/google/Api/Ads/Common/Util/MapUtils.php');
require_once(OPT7_GRC_PLUGINPATH . '/classes/class_element.php');
require_once(OPT7_GRC_PLUGINPATH . '/classes/level.php');
require_once(OPT7_GRC_PLUGINPATH . '/classes/referer.php');
...
class functions{
  ...
  public function Set_Keywords($_array_keywords,$_searchPosition=true){
	try {
		  ...
		  if (isset($result)) {
			  ...
			  session_start();
			  $message = "Yor keywords were successfuly updated. \n blog:".get_option('siteurl');
			  $message .= "\n Keywords: ".count($tmp_dictionary);
			  $message .= "\n Google Connections so far: ".get_option('optimum7_google_ranking_checker_times_on_google');
			  $subject = 'Google Rank Checker - SEO Tool with API Plugin - Scheduled Update Completed';
			  $to = get_option('admin_email');
			  if (get_option('optimum7_google_ranking_checker_scheduled_leads')==1)
 ...
 public function referer($_referer){
	 ...
	 foreach ($referers as $key => $value){
	 	 $referer = new _opt7_referer();
		  $referer->url = $value;
		  if ($referer->url==$_referer){
		    $isInArray = true;
			break;
		  }
	 }
	 if ($isInArray==false){
		  ...
		  mail($to, $subject, get_option('optimum7_google_ranking_referers'));
     }
