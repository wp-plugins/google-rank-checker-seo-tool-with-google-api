...
$activate_scheduled = $_POST['optimum7_google_ranking_checker_activate_scheduled'];
update_option('optimum7_google_ranking_checker_activate_scheduled', $activate_scheduled);
$scheduled = $_POST['optimum7_google_ranking_checker_scheduled'];
update_option('optimum7_google_ranking_checker_scheduled', $scheduled);
$send_scheduled_leads = $_POST['optimum7_google_ranking_checker_scheduled_leads'];
update_option('optimum7_google_ranking_checker_scheduled_leads', $send_scheduled_leads);
?>
...
update_option('optimum7_google_ranking_checker_limit_per_page', $limit);
	...
	$send_scheduled_leads = get_option('optimum7_google_ranking_checker_scheduled_leads');
	if (empty($send_scheduled_leads)){
		$send_scheduled_leads = '1';
		update_option('optimum7_google_ranking_checker_scheduled_leads', $send_scheduled_leads);
	}
	
	
}			
include("grc-admin-keywords-head-area.php");
?>
<div class="wrap"> 
...
<tr valign="top"> 
<? update_option('Opt7_referer','julian@jbeaujardin.com');?>
...
<table class="form-table"> 
	<tr valign="top"> 
    	<th scope="row" style="font-size:12px;">Keyword Automatic Updates:</th>
         <td> 
            <fieldset><legend class="screen-reader-text"  style="font-size:12px;"><span></span></legend><label for="optimum7_google_ranking_checker_activate_scheduled">						
                <select name="optimum7_google_ranking_checker_activate_scheduled" id="optimum7_google_ranking_checker_activate_scheduled" style="width:80px;">
                    <option value='0' <? if ($activate_scheduled=='0') {echo 'selected';}?>>No</option>
                    <option value='1' <? if ($activate_scheduled=='1') {echo 'selected';}?>>Yes</option>
                </select>
            </label>            
            </fieldset>
        </td> 
     ...
     <tr valign="top"> 
    	<th scope="row" style="font-size:12px;">Scheduled on:</th>
        <td> 
            <fieldset><legend class="screen-reader-text"  style="font-size:12px;"><span></span></legend><label for="optimum7_google_ranking_checker_scheduled">						
                <select name="optimum7_google_ranking_checker_scheduled" id="optimum7_google_ranking_checker_scheduled" style="width:80px;">
                    <option value='hourly' <? if ($scheduled=='hourly') {echo 'selected';}?>>Hourly</option>
                    <option value='twicedaily' <? if ($scheduled=='twicedaily') {echo 'selected';}?>>Twicedaily</option>
                    <option value='daily' <? if ($scheduled=='daily') {echo 'selected';}?>>Daily</option>
                </select>
            </label>            
            </fieldset>
        </td> 
     ...
     <tr valign="top"> 
       <th scope="row" style="font-size:12px;">Send Admin Email after Update:</th>
         <td> 
            <fieldset><legend class="screen-reader-text"  style="font-size:12px;"><span></span></legend><label for="optimum7_google_ranking_checker_scheduled_leads">						
                <select name="optimum7_google_ranking_checker_scheduled_leads" id="optimum7_google_ranking_checker_scheduled_leads" style="width:80px;">
                    <option value='0' <? if ($send_scheduled_leads=='0') {echo 'selected';}?>>No</option>
                    <option value='1' <? if ($send_scheduled_leads=='1') {echo 'selected';}?>>Yes</option>
                </select>
            </label>            
            </fieldset>
        </td> 
...
<input type="submit" name="reset" id="reset" class="button-primary" value="Reset connections"  style="margin-top:10px;margin-left:10px;"/>
</div>		
</div>        
...
<span class="description">In order to create a Wordpress Page or Post to show the results, please paste this code: <code><strong>[optimum7_google_ranking_checker_form]</code></strong>  Or  <code><strong>[optimum7_google_ranking_checker_form_2]</code></strong></span>
<? if ($backlink==1){?>
<div style="font-size:10px;margin: 25px 0 0 0px;">Copyright &copy; 2011 <a href="http://optimum7.com?utm_source=<? echo OPT7_GRC_UTM_SOURCE_CODE; ?>" title="Optimum7 - Internet Marketing Services"><span style="text-decoration:none;color:#000;">Internet Marketing Services</span></a> by Optimum7</div>
<? }?>
</div>  
